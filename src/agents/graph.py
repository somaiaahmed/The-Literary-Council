...
# ─── Gemini helper ────────────────────────────
async def call_gemini(system_prompt: str, scenario: str, model: str = "gemini-3-flash-preview") -> str:
...
"""
    ...
    content = await call_gemini("You are a master synthesizer.", prompt, model="gemini-3-flash-preview")
...
def get_agent_definitions() -> list[dict]:
    ...
    return [
        {
            "agent_key": a["key"],   # ← renamed to match what frontend expects
            "agent_name": a["name"],
            "book": a["book"],
            "color": a["color"],
            "icon": a["icon"],
